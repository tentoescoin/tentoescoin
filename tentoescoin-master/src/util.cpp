...
#else
    const char* pszModule = "tentoescoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "tentoescoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No tentoescoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override tentoescoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tentoescoin.pid"));
